...
--params = {closeWhenShow = true,fixPri = true} 打开时关闭、关闭特定pri的dialog
function display.closeDialog(pri, params)
    pri = pri or 1
    local cpri,cdialog
    for i=_dialogDepth, 1, -1 do
        if _dialogs[i] then
            cpri = _dialogs[i].priority or 1
            local closeCondition = cpri>=pri
            local fixPri = params and params.fixPri
            if fixPri then
                closeCondition = (cpri == pri)
            end
            if closeCondition then
                cdialog = _dialogs[i]
                ...
                local t = 0
                --修改下 最上层的dialog才会动态退出
                if i==_dialogDepth then
                ...
                if (not params or not params.closeWhenShow) and not cdialog.swallow then
                    Event.sendEvent(Event.EventDialogClose, true)
                end
                if pri==0 then break end
            elseif not fixPri then
                break
            end
        end
    end
...
function display.showDialog(dialog, autoPop, fullscreen, opPct)
    ...
    if _dialogDepth>0 then
        ...
        display.closeDialog(pri, {closeWhenShow = true})
    end
    _dialogDepth = _dialogDepth+1
    ...
    elseif not fullscreen then
        ...
        dialog.view:runAction(ui.action.sequence({{"scaleTo",t/2,ts*1.05,ts*1.05},{"scaleTo",t/2,ts*0.95,ts*0.95},{"scaleTo",t/3,ts,ts}}))
    end
    if dark then
...
function display.pushNotice(text,params)
    ...
    local label = ui.label(text, setting.fontName or General.font1, (setting.fontSize or 30)*nscale, {width=750, color=color})
        
    display.adapt(label, 0, CC_WINSIZE.height*0.5*(setting.yRate or 0.5), GConst.Anchor.Top, {datum=GConst.Anchor.Center})
    ...
    --     yRate = setting.yRate,
    --     disy = disy,
    --     yPos = CC_WINSIZE.height*0.5*(setting.yRate or 0.5)
    --     })
    ...
    for _, label in pairs(_notices) do
        ...
        --label:runAction(ui.action.sequence({{"moveBy",0.1,0,disy+my},"remove"}))
...
function display.sendIntent(intent)
    ...
    if intentType == "dialog" then
        ...
        if not intent.autoShowed then
            local autoPop = intent.autoPop or newDialog._autoPop
            if autoPop == nil then
                autoPop = true
            end
            display.showDialog(newDialog, autoPop)
        end
    end
end
