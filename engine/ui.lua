...
function UI.sprite(filename, size, keepScale, lazyLoad)
    ...
    if not sprite then
        ...
        sprite = CaeSprite:create("UIImages/icon/cm_btnMap.png")
    end
    return sprite
end
...
function UI.scale9(filename, edge, size)
    local sprite = CaeSprite:create(filename)
...
function UI.animateSprite(duration, prefix, frameNum, setting)
    --print("ui.animate",prefix..setting.beginNum..".png")
...
function UI.textBox(size, text, fontId, fontSize, setting)
    ...
    if not caeFont then
        log.e(debug.traceback())
        log.e("Error in use label font:" .. fontId)
        return nil
    end
...
function UI.label(text, fontName, fontSize, setting)
    ...
    if not caeFont then
        log.e(debug.traceback())
        log.e("Error in use label font:" .. fontName)
    end
    ...
    if outColor then
        ...
        label:setShadowColor(cc.c4b(outColor[1],outColor[2],outColor[3],outColor[4] or 255))
    end
    -- label:setGlobalZOrder(2)
    --label:setString(text)
    -- local bg = ui.colorNode({width,height},{100,100,100})
    -- label:addChild(bg,-1)
...
function UI.button(size, callback, params)
    ...
    if params.image == "images/dialogButNext.png" then
        but:setBackgroundSound("sounds/dialogButNext.mp3")
    end
    return but
end
...
function UI.simpleCsbEffect(csb,loop,b)
